...
public class OperationLogAspect {
     ...
     */
    @Pointcut("@annotation(cn.master.backend.handler.annotation.Log)")
    ...
    public void before(JoinPoint joinPoint) {
        try {
            ...
            if (msLog != null && isMatch(msLog.type())) {
                ...
                for (Class<?> clazz : msLog.logClass()) {
                    context.setVariable("logClass", applicationContext.getBean(clazz));
                    isNext = true;
                }
                if (!isNext) {
                    return;
