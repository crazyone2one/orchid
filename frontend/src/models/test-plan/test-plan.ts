import type {customFieldsItem} from '/@/models/case-management/feature-case.ts';
import type {TableQueryParams} from '/@/models/common.ts';
import {BatchApiParams, DragSortParams} from '/@/models/common.ts';
import {LastExecuteResults} from '/@/enums/case-enum.ts';
import {type PlanMinderAssociateType, testPlanTypeEnum,} from '/@/enums/test-plan-enum.ts';
...
export interface SwitchListModel {
    ...
    label: string;
    desc: string[];
    tooltipPosition: 'top-start' | 'top' | 'top-end' | 'right-start' | 'right' | 'right-end' | 'bottom-start' | 'bottom' | 'bottom-end' | 'left-start' | 'left' | 'left-end';
}
