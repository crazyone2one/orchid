...
import {ContentTabsMap, CustomAttributes, TabItemType} from "/@/models/case-management/feature-case.ts";
...
import {isArraysEqualWithOrder} from "/@/utils";
import useLocalForage from "/@/hooks/use-local-forage.ts";
...
const useFeatureCaseStore = defineStore('featureCase', {
    ...
    actions: {
        ...
        async setContentTabList(arr: TabItemType[]) {
            ...
            const tabsMap = await getItem<ContentTabsMap>('caseTabsMap');
            if (tabsMap) {
                tabsMap.tabList = JSON.parse(JSON.stringify(arr));
                await setItem('caseTabsMap', tabsMap);
            }
        },
    }
