...
import {addUserToOrgOrProject} from "/@/api/modules/setting/system-org-project.ts";
...
const {
  ...
  form, send: submit, reset: resetForm
    ...
    formData => {
      ...
      return addUserToOrgOrProject({userIds: formData?.name, projectId, organizationId});
    },
    {
      // 初始化表单数据
      initialForm: {
    ...
    <div>
      <n-form ref="formRef" :model="form">
        <n-form-item :label="$t('system.organization.member')" path="name"
          ...
          <user-select v-model="form.name" :type="UserRequestTypeEnum.SYSTEM_ORGANIZATION" mode="remote"
